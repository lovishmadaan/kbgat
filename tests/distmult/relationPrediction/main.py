...
def batch_gat_loss(gat_loss_func, train_indices, entity_embed, relation_embed):
    ...
    # Distmult
    pos_norm_distmult = torch.sum((source_embeds * relation_embeds) * tail_embeds, dim=1)
    ...
    # Distmult
    neg_norm_distmult = torch.sum((source_embeds * relation_embeds) * tail_embeds, dim=1)

    y = -torch.ones(int(args.valid_invalid_ratio_gat) * len_pos_triples).cuda()

    loss = gat_loss_func(-pos_norm_distmult, -neg_norm_distmult, y)
    return loss
